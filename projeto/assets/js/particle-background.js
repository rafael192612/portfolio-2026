export function initParticles() {
  ...
  let lightGrowthTimer = 0;
  let lightSizePercent = 30;

  function setLightSize(percent, duration = "0.9s") {
    if (lightSizePercent === percent) return;
    lightSizePercent = percent;
    const scale = percent / 100;
    document.documentElement.style.setProperty("--cursor-light-size-duration", duration);
    document.documentElement.style.setProperty("--cursor-light-min-radius", `${(8.05 * scale).toFixed(3)}rem`);
    document.documentElement.style.setProperty("--cursor-light-fluid-radius", `${(14 * scale).toFixed(2)}vw`);
    document.documentElement.style.setProperty("--cursor-light-max-radius", `${(14 * scale).toFixed(2)}rem`);
  }

  function lightReturnDuration() {
    const growthProgress = Math.max((lightSizePercent - 30) / 70, 0);
    return `${(2 + growthProgress * 1.5).toFixed(2)}s`;
  }

  function restartLightGrowth() {
    window.clearInterval(lightGrowthTimer);
    setLightSize(30, lightReturnDuration());
    lightGrowthTimer = window.setInterval(() => {
      if (lightSizePercent >= 100) {
        window.clearInterval(lightGrowthTimer);
        lightGrowthTimer = 0;
        return;
      }

      setLightSize(Math.min(lightSizePercent + 10, 100));
    }, 1000);
  }
  ...
  function particleCount() {
    ...
    const minimum = 22;
    const maximum = 180;
    const areaPerParticle = 18000;
    return Math.min(Math.max(Math.round(area / areaPerParticle), minimum), maximum);
  }
  ...
  function makeParticle() {
    ...
    return {
      ...
      baseVx: vx,
      baseVy: vy,
      size: (0.55 + Math.random() * 0.8) * depth * 2.15,
      depth,
  ...
  function interactionRadius() {
    const baseRadius = Math.min(Math.max(width * 0.105, 104), 160);
    const growthProgress = Math.max((lightSizePercent - 30) / 70, 0);
    return baseRadius * (1 + growthProgress * 0.8);
  }
  ...
  function drawConnections() {
    ...
    for (let first = 0; first < particles.length; first += 1) {
      for (let second = first + 1; second < particles.length; second += 1) {
        ...
        const dx = a.x - b.x;
        const dy = a.y - b.y;
        if (Math.abs(dx) > baseDistance * 1.35 || Math.abs(dy) > baseDistance * 1.35) continue;
        const distance = Math.hypot(dx, dy);
        ...
        const opacity = distanceOpacity * (lightTheme
          ? 0.2 + lightInfluence * 0.2
          : 0.17 + lightInfluence * 0.34);
        const lineBase = lightTheme ? [5, 47, 242] : [226, 226, 222];
        const lineNear = lightTheme ? [5, 47, 242] : [247, 246, 242];
        const lineRed = Math.round(lineBase[0] + (lineNear[0] - lineBase[0]) * lightInfluence);
        const lineGreen = Math.round(lineBase[1] + (lineNear[1] - lineBase[1]) * lightInfluence);
        const lineBlue = Math.round(lineBase[2] + (lineNear[2] - lineBase[2]) * lightInfluence);

        context.beginPath();
        context.moveTo(a.x, a.y);
        context.lineTo(b.x, b.y);
        context.strokeStyle = `rgb(${lineRed} ${lineGreen} ${lineBlue} / ${opacity})`;
        context.lineWidth = interactive ? 0.9 : 0.62;
        context.stroke();
      }
    }
  }
  ...
  function drawParticles(time) {
    ...
    particles.forEach((particle, particleIndex) => {
      ...
      const pulse = 0.94 + Math.sin(time * 0.00045 + particle.phase) * 0.06;
      const depthVisibility = 0.92 + particle.depth * 0.08;
      const opacity = (lightTheme ? 0.94 + influence * 0.04 : 0.56 + influence * 0.34) * pulse * depthVisibility;
      const particleBase = lightTheme ? [5, 47, 242] : [226, 226, 222];
      const particleNear = lightTheme ? [5, 47, 242] : [250, 249, 246];
      const particleRed = Math.round(particleBase[0] + (particleNear[0] - particleBase[0]) * influence);
      const particleGreen = Math.round(particleBase[1] + (particleNear[1] - particleBase[1]) * influence);
      const particleBlue = Math.round(particleBase[2] + (particleNear[2] - particleBase[2]) * influence);
      ...
      if (influence > 0.04) {
        ...
        halo.addColorStop(0, `rgb(${particleRed} ${particleGreen} ${particleBlue} / ${influence * 0.26})`);
  ...
  function start() {
    ...
    if (document.hidden || reducedMotion.matches) {
      clearInterval(lightGrowthTimer);
      lightGrowthTimer = 0;
  ...
  window.addEventListener("vertex:themechange", () => {
